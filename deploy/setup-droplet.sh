...
echo ""
echo "  2. Run jinn setup as $APP_USER (first time only):"
echo "     sudo -u $APP_USER $APP_DIR/node_modules/.bin/jinn setup"
echo "     - This creates ~/.jinn/ and installs agents/config"
echo ""
echo "  3. Start PM2 as $APP_USER (NOT as root):"
echo "     sudo -u $APP_USER pm2 start $APP_DIR/deploy/ecosystem.config.cjs"
...
echo "  as root. Always start PM2 as the $APP_USER user."
echo ""
echo "  4. Check status:"
echo "     sudo -u $APP_USER pm2 status"
echo "     sudo -u $APP_USER pm2 logs gentech-agents"
echo ""
